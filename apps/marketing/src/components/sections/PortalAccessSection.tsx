      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-12 grid gap-6">
          <div className="relative mx-auto w-full max-w-2xl rounded-2xl border border-subtle surface-1 p-8 overflow-hidden">
            <div className="absolute -top-20 -right-20 h-60 w-60 rounded-full" style={{ background: "var(--gradient-glow)" }} />
            <div className="relative">
              ...
              <p className="mt-3 text-sm text-muted-foreground">For Columbus AI clients. Use this to:</p>
              <ul className="mt-4 space-y-2 text-sm text-muted-foreground">
                {["View website performance", "Track leads and automations", "Submit requests", "View reports", "Manage billing", "Access documents"].map((i) => (
                  <li key={i} className="flex gap-2"><span className="text-[color:var(--brand-cyan)]">→</span>{i}</li>
                ))}
              </ul>
              <Button asChild className="mt-6 bg-gradient-primary text-primary-foreground">
                <Link to="/client">Open Client Portal <ArrowRight className="ml-2 h-4 w-4" /></Link>
              </Button>
            </div>
          </div>
        </div>
