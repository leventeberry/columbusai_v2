...
const workflows = [
  {
    trigger: "New Lead",
    action: "Instant Response + CRM Routing",
    outcome: "Cut response time from hours to seconds.",
  },
  {
    trigger: "Missed Call",
    action: "Text Back + Booking Link",
    outcome: "Recover revenue from missed connections.",
  },
  {
    trigger: "Appointment Scheduled",
    action: "Reminder Sequence",
    outcome: "Fewer no-shows, fuller calendars.",
  },
  {
    trigger: "Job Completed",
    action: "Review Request",
    outcome: "Compound social proof on autopilot.",
  },
  {
    trigger: "Invoice Paid",
    action: "Onboarding Workflow",
    outcome: "New customers start without manual handoff.",
  },
  {
    trigger: "Support Request",
    action: "Categorize + Assign",
    outcome: "Right team, right ticket, every time.",
  },
  {
    trigger: "Daily Ops",
    action: "Summary to Email or Slack",
    outcome: "One scan a day keeps surprises away.",
  },
  {
    trigger: "Website Form",
    action: "Portal Request + Notification",
    outcome: "Inbound work tracked from the first click.",
  },
];
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-14 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {workflows.map((w) => (
            <div
              key={w.trigger}
              className="rounded-2xl border border-subtle surface-1 p-5 hover:border-strong transition-colors"
            >
