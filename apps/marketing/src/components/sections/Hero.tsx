...
const stats = [
  { label: "Business Impact", value: "$48.2k", sub: "+18% MoM", icon: TrendingUp, accent: "text-[color:var(--success)]" },
  { label: "Active Automations", value: "127", sub: "All healthy", icon: Zap, accent: "text-[color:var(--brand-violet)]" },
  { label: "Website Status", value: "Operational", sub: "99.99% uptime", icon: Globe, accent: "text-[color:var(--brand-cyan)]" },
  { label: "Leads Captured", value: "342", sub: "this month", icon: Users, accent: "text-[color:var(--brand-indigo)]" },
  { label: "Client Requests", value: "12 open", sub: "4 in review", icon: Workflow, accent: "text-[color:var(--warning)]" },
  { label: "Workflow Health", value: "98.7%", sub: "success rate", icon: Activity, accent: "text-[color:var(--success)]" },
];
      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pt-20 pb-24 lg:pt-28 lg:pb-32">
        ...
        >
          ...
          <p className="mt-6 text-lg text-muted-foreground max-w-2xl leading-relaxed">
            Columbus AI builds and manages automation systems, client portals, websites, integrations,
            and operational workflows that help businesses respond faster, save time, and scale without chaos.
          </p>
          <div className="mt-8 flex flex-wrap gap-3">
            ...
            <Button asChild size="lg" variant="outline" className="border-strong bg-card/40 backdrop-blur">
  ...
  return (
    <div className="relative rounded-2xl border border-strong surface-1 shadow-elevated overflow-hidden">
      ...
      <div className="flex items-center gap-2 px-4 py-3 border-b border-subtle surface-2">
        ...
        <div className="flex-1 text-center text-xs text-muted-foreground font-mono">columbusai.tech/console</div>
      </div>

      <div className="p-4 sm:p-6">
        <div className="flex items-center justify-between mb-5">
          <div>
            <div className="text-xs text-muted-foreground uppercase tracking-wider">Operations Overview</div>
            <div className="text-lg font-semibold text-foreground mt-0.5">Today · All systems nominal</div>
          </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
          {stats.map((s) => (
            <div key={s.label} className="rounded-xl border border-subtle surface-2 p-4 hover:border-strong transition-colors">
              <div className="flex items-center justify-between">
                <span className="text-xs text-muted-foreground">{s.label}</span>
                <s.icon className={`h-3.5 w-3.5 ${s.accent}`} />
              </div>
              <div className="mt-2 text-2xl font-semibold text-foreground tracking-tight">{s.value}</div>
