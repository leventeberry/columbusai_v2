/**
 * Phase 6: In-memory metrics for /api/chat. Module-level singleton.
 * Counters and simple latency histogram; no Prometheus/OTel.
 */
...
export function getMetricsSnapshot(): {
  ...
  return {
    chat_requests_total: state.chat_requests_total,
    chat_429_total: state.chat_429_total,
    chat_5xx_total: state.chat_5xx_total,
    chat_2xx_total: state.chat_2xx_total,
    latency_ms_avg: state.latency_ms_count ? state.latency_ms_sum / state.latency_ms_count : 0,
    latency_ms_count: state.latency_ms_count,
    latency_ms_buckets: { ...state.latency_ms_buckets },
  };
}
