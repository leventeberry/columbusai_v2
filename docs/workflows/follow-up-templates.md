...
Columbus’s live env-driven workflow ([follow-up-workflow.md](follow-up-workflow.md)) is separate. Do **not** activate template workflows on Columbus unless you intentionally migrate.
...
7. Ensure n8n has `N8N_BLOCK_ENV_ACCESS_IN_NODE=false` and `BOOKING_LINK` set (see [follow-up-workflow.md](follow-up-workflow.md)).
