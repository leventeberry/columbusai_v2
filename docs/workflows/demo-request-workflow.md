...
Marketing `ContactSection` → `POST {VITE_API_URL}/api/leads/demo` → `@columbusai/leads` `processDemoLead` → Postgres `leads` table (or `LEADS_PATH` jsonl) → n8n demo-request webhook (non-blocking). Follow-up emails are sent by the scheduled n8n workflow (see [follow-up-workflow.md](follow-up-workflow.md)).

Shared lead logic lives in [`packages/leads`](../../packages/leads/) (ported from marketing server fn + n8n sender).

Discovery-call follow-up: [follow-up-workflow.md](follow-up-workflow.md).
...
Version and deploy the demo workflow from the repo: see [n8n-workflow-sync.md](../architecture/n8n-workflow-sync.md) (`pnpm n8n:list`, `n8n:pull:demo`, `n8n:push:demo`, `n8n:activate:demo`).
